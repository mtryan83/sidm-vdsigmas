...
lmin = lambda beta, kappa: np.maximum(1./2.,beta*kappa)
lminp = lambda beta, kappa: np.maximum(1.,2.*beta*kappa)
turn = lambda beta, betalow, a: exp(-(np.maximum(beta, betalow) - betalow)*a)
...
zeta = lambda kappa, beta, lmin: (np.maximum(lmin, beta*kappa)**2 - lmin**2)/(2*kappa**2*beta**2) + eta(np.maximum(lmin, beta*kappa)/kappa)
...
def sigmaTatt(beta, kappa):
  ...
  sig = np.zeros_like(kappa)
  betasm = beta < 1
  betalg = beta > 50
  inter = (~betasm)&(~betalg)
  sig[betasm] = sigmaT_smallbeta(beta[betasm],kappa[betasm])*turn(beta[betasm],0.2,-0.64)
  sig[betalg] = 2. * log(beta[betalg]) * (log(log(beta[betalg])) + 1)
  sig[inter] = 4.7*log(beta[inter] + 0.82)
  return sig
...
def sigmaTrep(beta, kappa):
  ...
  betalg = beta > 50
  inter = (~betasm)&(~betalg)
  sig[betasm] = sigmaT_smallbeta(beta[betasm],kappa[betasm])*turn(beta[betasm],0.2,0.53)
  sig[betalg] = lambdaT * (log(2.*beta[betalg])-log(log(2.*beta[betalg])))**2.
  sig[inter] = 2.9*log(beta[inter] + 0.47)
  return sig
...
def sigmaVatt(beta, kappa, lmin):
  ...
  betalg = beta > 25
  inter = (~betasm)&(~betalg)
  sig[betasm] = sigmaV_smallbeta(beta[betasm],kappa[betasm],lmin)*turn(beta[betasm],0.1,-0.67)
  sig[betalg] = (1 + log(beta[betalg])- 1/(2.*log(beta[betalg])))**2/2.
  sig[inter] = 2.5*log(beta[inter] + 1.05)
  return sig
...
def sigmaVrep(beta, kappa, lmin):
  ...
  betalg = beta > 25
  inter = (~betasm)&(~betalg)
  sig[betasm] = sigmaV_smallbeta(beta[betasm],kappa[betasm],lmin)*turn(beta[betasm],0.1,0.370562)
  sig[betalg] = log(2. * beta[betalg]) * (lambdaV * log(2. * beta[betalg]) - (2.*lambdaV - 1) * log(log(2.*beta[betalg])))
  sig[inter] = 2.8*log(beta[inter] + 0.80)
  return sig
...
classics_folder = pathlib.Path(__file__).parent.resolve()

for mode in modes:
  for sign in signs:

    outputname_data = classics_folder / ('sigma'+mode+'list_'+sign+'.txt')
...
def sigma(kappa, beta, mode = 'T', sign = 'attractive'):
  if not(sign == 'attractive' or sign == 'repulsive'):
    ...
    # exit()
    raise ValueError(f'Sign {sign} not recognized in function sigma()')
  if np.any(kappa < 1):
...
def sigma_combined(kappa,beta,mode = 'T', sign = 'attractive'):
    ...
    sig = np.zeros_like(kappa)
    sig[classical] = sigma(kappa[classical],beta[classical],mode,sign)
    sig[quantum] = sigma_Hulthen(kappa[quantum],np.minimum(beta[quantum],1e6),mode,sign)
    sig[inter] = (1-kappa[inter])/0.6*sigma_Hulthen(0.4,np.minimum(beta[inter],1e6),mode,sign) + (kappa[inter]-0.4)/0.6*sigma(np.ones_like(kappa[inter]),beta[inter],mode,sign)
    return sig
